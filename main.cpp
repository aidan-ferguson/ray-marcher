...
const unsigned int N_FRAMES = 30;
...
int march_ray(glm::vec3 ray_dir, float time){
    ...
    // We also need to rotate the direction to point at the center, however the rotation matrix is going to be 180 degrees out of phase
    // glm::mat3 dir_rot_mat = {
    //     {std::cos(time - M_PI) , 0, std::sin(time - M_PI)},
    //     {0              , 1, 0},
    //     {-std::sin(time - M_PI), 0, std::cos(time - M_PI)}
    // };
